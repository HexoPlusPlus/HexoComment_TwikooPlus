...
document.getElementById(id).innerHTML=`<div id='hpp_comment_twikoo'><\/div>`
var ajax = ajaxObject();
ajax.open('get',`https://cdn.jsdelivr.net/npm/twikoo@${ver}/dist/twikoo.all.min.js`,true);
ajax.onreadystatechange = function(){
    if( ajax.readyState == 4 ){
        if( ajax.status >=200){
			...
			twikoo.init({envId: envId,el: '#hpp_comment_twikoo',region: region,path: path})
        }
    }
};
ajax.send();
}
function use_twikooPlus(id,domain,path_r,cdn,adminmail,friendmail,guestcolor,friendcolor,admincolor,envId,region,ver){
path=eval(path_r)
//console.log('Use TwikooPlus')
back=`https://${domain}/hpp/api/twikoo`
let before=0
var ajax = ajaxObject();
    ...
    ajax.onreadystatechange = function () {
        if( ajax.readyState == 4 ) {
			...
			let count=JSON.parse(JSON.parse(ajax.responseText)["data"]["response_data"])["count"]
			let res=JSON.parse(JSON.parse(ajax.responseText)["data"]["response_data"])["data"]
			localStorage.setItem("hpp_twikooplus_count",count)
			for(var i=0;i<getJsonLength(res);i++)
			{count--;
				if(res[i]["isSpam"]!=true){
					let idman="路人";
					let color=guestcolor;
					if(res[i]['mailMd5']==adminmail){idman='管理员';color=admincolor};
					for(var j=0;j<getJsonLength(friendmail);j++){if(res[i]["mailMd5"]==friendmail[j]){idman='小伙伴';color=friendcolor}}
    ...
    <div id="${res[i]['id']}"> 
     <img class="hpp_twikooplus_avatar" src="${cdn}${res[i]['mailMd5']}" alt="${res[i]['nick']}" width="80" height="80" /> 
...
<div class="hpp_twikooplus_comment-children" id="${res[i]['id']}_child"></div>	 
	...
	for(var k=0;k<getJsonLength(res[i]['replies']);k++){
		...
		let idman="路人";
		let color=guestcolor;
		if(res[i]['replies'][k]['mailMd5']==adminmail){idman='管理员';color=admincolor};
					for(var j=0;j<getJsonLength(friendmail);j++){if(res[i]['replies'][k]["mailMd5"]==friendmail[j]){idman='小伙伴';color=friendcolor}}
    ...
    <div id="${res[i]['replies'][k]['id']}"> 
     <img class="hpp_twikooplus_avatar" src="${cdn}${res[i]['replies'][k]['mailMd5']}" alt="${res[i]['replies'][k]['nick']}" width="80" height="80" /> 
     <div class="hpp_twikooplus_comment-main"> 
      <p class="hpp_twikooplus_comment-at"></p>
...
<div class="hpp_twikooplus_comment-children" id="${res[i]['replies'][k]['id']}_child"></div>	 
  ...
  Powered by <a  href="https://hexoplusplus.js.org" target="_blank">HexoPlusPlus_TwikooPlus</a>${ver}</div>`
				...
				document.getElementById("hpp_twikooplus_comment_list").innerHTML+=`<button class="hpp_comment_loadmore">莫得更多了！</button><div class="hpp_twikooplus-footer">
  Powered by <a  href="https://hexoplusplus.js.org" target="_blank">HexoPlusPlus_TwikooPlus</a>${ver}</div>`
			}
			localStorage.setItem("hpp_twikooplus_count",count)
            }
    ...
    ajax.send(JSON.stringify(comment_init));
}

function hpp_comment_loadmore(id,domain,path_r,cdn,adminmail,friendmail,guestcolor,friendcolor,admincolor,before,envId,ver,region){
path=eval(path_r)
	count=localStorage.getItem("hpp_twikooplus_count")
	console.log(id)
document.getElementById("hpp_twikooplus_comment_list").innerHTML=`<p style="text-align:center">正在加载下一页| <strong><a href="javascript:use_twikoo('${envId}','${id}','${region}','${path_r}','${ver}')">强制使用原版</a></strong></p></p><div class="hpp_twikooplus-footer">
  Powered by <a  href="https://hexoplusplus.js.org" target="_blank">HexoPlusPlus_TwikooPlus</a>${ver}</div>`
back=`https://${domain}/hpp/api/twikoo`
var ajax = ajaxObject();
    ajax.open( "post" , back , true );
...
<div class="hpp_twikooplus_comment-children" id="${res[i]['replies'][k]['id']}_child"></div>	 
  ...
  Powered by <a  href="https://hexoplusplus.js.org" target="_blank">HexoPlusPlus_TwikooPlus</a>${ver}</div>`
				...
				document.getElementById("hpp_twikooplus_comment_list").innerHTML+=`<button class="hpp_comment_loadmore">莫得更多了！</button>`
			}
			localStorage.setItem("hpp_twikooplus_count",count)
            }
            else {
...
function hpp_comment({id,envId,region,ver,domain,path,cdn,adminmail,friendmail,guestcolor,friendcolor,admincolor,timelimit,mode}){
if(mode==1){
document.getElementById(id).innerHTML=`<p style="text-align:center">检测当前环境中| <strong><a href="javascript:use_twikoo('${envId}','${id}','${region}','${path}','${ver}')">强制使用原版</a></strong></p></p>`
let locationr=""
var ajax = ajaxObject();
    ajax.open( "post" , "https://"+domain+"/cdn-cgi/trace" , true );
    ajax.setRequestHeader( "Content-Type" , "text/plain" );
    ajax.onreadystatechange = function () {
        if( ajax.readyState == 4 ) {
			if( ajax.status == 200 ) {
				locationr=ajax.responseText.split('loc=')[1].split('\n')[0]
